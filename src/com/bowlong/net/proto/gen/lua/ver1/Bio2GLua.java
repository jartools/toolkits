package com.bowlong.net.proto.gen.lua.ver1;
...
public class Bio2GLua {
	...
	public static String b2g(Class<?> c, boolean src) {
		...
		sb.pn("do");
		sb.pn("  ArrayList = luanet.import_type('System.Collections.ArrayList');");
		sb.pn("  Hashtable = luanet.import_type('System.Collections.Hashtable');");
		// sb.pn("  require('CLLNetDispatch');");
		sb.pn("");
		sb.pn("  " + sname + " = {");
		sb.pn("");
		// =========== 生成实体对象
	...
	public static void g2bean(Class<?> c, String namespace, StrBuilder sb,
		...
		for (Field field : fs) {
			...
			if (field.getType().equals(List.class)) {
				...
				boolean isBtype = B2G.isBType(gtype);
				if (gtype != null && !gtype.isEmpty() && !isBtype) {
					sb.pn("        r.${1} = ${2}.${3}.maps_${1}(dataMap[\"${4}\"]);--${4}",
							s, fathName, cname, hs, t);
				} else {
					sb.pn("        r.${1} = dataMap[\"${2}\"];  ${3}", s,
							hs, remark);
				}
			} else {
				if (gm.equals("getObject")) {
					sb.pn("        r.${1} = ${2}.${3}.parse(dataMap(\"${4}\"));  ${5}",
	...
	public static void lua_request(Class<?> c, String namespace, StrBuilder sb,
		...
		for (Method m : methods) {
			...
			sb.pn("        _map:Add(\"-100\", ${1}.callNet.__sessionid);-- __sessionid",
					fathName);
			sb.pn("        _map:Add(\"${1}\", ${2});-- cmd:${3}",
			...
			for (NewMap<String, String> m1 : params) {
				...
				if (isOut) {
					...
					if (p.equals("getList")) {
						...
						sb.pn("        _map:Add(\"${1}\", ${2}.toMap());",
								hval, val);
					} else {
						sb.pn("        _map:Add(\"${1}\", ${2});", hval,
								val);
					}
				}
			}
	...
	public static void lua_call(Class<?> c, String namespace, StrBuilder sb,
		...
		sb.pn("        local cmd = map[\"${1}\"];", B2G.METHOD);
